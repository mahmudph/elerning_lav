...
$(document).on("click",".edit",function() {
  ...
  $('#msg-content').hide();
  /* url  */
  let url = `{{ config('app.url')}}/admin/pengajaran-pelajaran-kelas/${id}/edit`;
  /* request modal */
...
$(document).on('click', '.delete', function() {
  let id  = $(this).attr('id_target');
  let url = `{{ config('app.url')}}/admin/pengajaran-pelajaran-kelas/${id}/delete`;
...
$(document).on('click', '#tambah', function() {
  let url = `{{ route('admin.pengajaran-pelajaran-kelas.create') }}`;
  /* change size modal */
  $('.modal-dialog').addClass('modal-lg');
  $('#msg-content').hide();

...
$(document).on("click",".edit",function() {
  ...
  /* url  */
  let url = `{{ config('app.url')}}/guru/data_tugas/${id}/edit`;
  /* request modal */
...
$(document).on('click', '.delete', function() {
  let id  = $(this).attr('id_target');
  let url = `{{ config('app.url')}}/guru/data_tugas/${id}/delete`;
...
$(document).on('click', '#tambah', function() {
  let url = `{{ route('guru.data_tugas.create') }}?id_pelajaran={{ $pelajaran->id }}`;
  /* change size modal */
  $('.modal-dialog').addClass('modal-lg');
  goAjaxGet(url).then(_ => {
    generateButton();
...
$(document).on('click', '.detail', function() {
  let id = $(this).attr('id_target');
  let url = `{{ route('guru.data_tugas.detail') }}/${id}`;
  $('.modal-dialog').addClass('modal-lg');
  goAjaxGet(url).then(_ => {
